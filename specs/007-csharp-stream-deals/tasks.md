...
- [X] T001 Resolve the uncommitted working-tree changes in `mt5_grpc_client_csharp/examples/NetStandardClientExample/Program.cs` and `mt5_grpc_client_csharp/examples/NetFramework48ClientExample/Program.cs` (changed server port, ad-hoc `ads`/`d1`/`a`/`a2` locals) — commit them or revert them, so the FR-011 example edits in Phase 6 do not land on top of debugging scratch. See [research.md](./research.md) Observation 3.
- [X] T002 Run `dotnet restore mt5_grpc_client_csharp/MetaTrader.Grpc.Client.sln` then `dotnet build mt5_grpc_client_csharp/MetaTrader.Grpc.Client.sln -c Debug` and confirm the regenerated `mt5_grpc_client_csharp/src/MetaTrader.Grpc.Client/obj/Debug/*/DealGrpc.cs` now contains a `StreamDeals` method on `TradeHistoryServiceClient` and that `Deal.cs` exposes `DealsRequest.ChunkSize`/`HasChunkSize`/`ClearChunkSize`. The checked-in `obj/` output predates the proto change and has `GetDeals` only; no authored contract work is required, only this rebuild.
...
- [X] T039 Confirm SC-005 explicitly: no pre-existing test, example or benchmark was modified to accommodate this change — `GeneratedSurfaceTests` still asserts 16 unary services / 31 unary RPCs, `UnaryWorkflowBenchmarks` and `PerformanceBudgetTests` are untouched, and the only edits to existing production files are the two added methods and the `GetDealsAsync` doc note in `Mt5GrpcClient.Trading.cs` plus the csproj metadata.
- [X] T040 Reconcile the publish trigger before tagging: `mt5_grpc_client_csharp/README.md` documents a `csharp-client-v<X.Y.Z>` tag but `.github/workflows/csharp-client-publish.yml` triggers on `v*.*.*` (shared with the server's Docker release). Pick one and make the workflow and README agree, or `5.1.0` will not publish as documented. See [research.md](./research.md) Observation 1.
  **Resolved in favour of the workflow, at the maintainer's direction.** The shared `v*.*.*` trigger is deliberate — commit `8e41b3e` moved both the workflow and `check-tag-version.ps1` off the client-scoped prefix on purpose, and `v5.0.0`-`v5.0.3` were published that way; the README was simply left stale. The README now documents `git tag v<X.Y.Z>` and both workflows are untouched.
  **Known consequence, documented rather than fixed:** the namespace is shared while the two artifacts version independently (server `0.x`, client `5.x`), so a tag matches only one of them. `v0.4.0` starts the client workflow, whose tag guard fails against `<Version>` `5.1.0` (nothing is published; the red run is expected), and `v5.1.0` starts the Docker release, publishing `mt5-grpc-server:5.1.0` — a server image numbered after the client. The README release section carries this warning.
...
### Status of the three tasks left open (recorded per Principle V / the constitution's "document any checks that could not be run")

- **T041 — NOT RUN.** Quickstart scenario 5 needs three things this implementation
  environment does not have: a running `mt5_grpc_server` at `0.4.0`+, an MT5 account
  with 1600+ closed deals, and a separate Windows 10 / .NET Framework 4.8 host. So
  **SC-001** (20 reads, zero server terminations), **SC-002 live spot check**,
  **SC-006** (native-channel pass from a `net48` host), **SC-007** (working-set
  observation) and **SC-008** (steady-state incremental cycle) are all
  **unverified against real transport**. What *is* verified in-process against the
  fake channel: multi-chunk delivery and ordering, unary/streaming parity across
  all six filter/group combinations (SC-002 behaviourally), prompt cancellation
  within 1 s (SC-004), one-chunk-at-a-time retention by construction (SC-007
  behaviourally), and `net48` reachability by compilation plus reflection (SC-006
  partially). Someone with the hardware must run scenario 5 before release and
  record the numbers in the PR.
- **T042 — GATE EVALUATED, CURRENTLY CLOSED.** No `0.4.0` release of
  `mt5_grpc_proto` / `mt5_grpc_server` exists yet: both `setup.py` files now read
  `0.4.0` in the working tree, but nothing is committed, tagged or published, and
  `git tag --list` carries no `0.4.0`. Per FR-016 / contract G4 the client `5.1.0`
  publish therefore **must not proceed yet**. All client-side verification above is
  complete and does not need to be repeated; only the publish step is blocked, until
  a `0.4.0` Python release satisfying `[0.4.0,1.0.0)` exists.
- **T043 — DECLINED, deliberately and on the record.** The hard-coded login,
  password, broker address and terminal path in
  `examples/NetFramework48ClientExample/Program.cs` were **left as they are** at the
  maintainer's explicit direction, to be handled as a separate change rather than
  folded into this feature. The constants are untouched by this work and remain
  committed credentials in a file this feature edits.
